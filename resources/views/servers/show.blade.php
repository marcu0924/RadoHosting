...
<div class="max-w-7xl mx-auto px-6 py-12 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-zinc-400 text-sm mt-1">
                Manage your server status, connection info, and console.
            </p>
        </div>

        <div class="flex gap-3">
            @php $isRunning = ($dockerStatus === 'running'); @endphp
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
            <h2 class="text-xl font-bold mb-4 text-white">Server Details</h2>
        ...
        <div class="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
            <h2 class="text-xl font-bold mb-4 text-white">Environment</h2>
    ...
    <section class="space-y-3">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                <label class="text-xs text-zinc-400">Filter:</label>
                ...
                <button id="refresh-console" type="button"
        ...
        <form action="{{ route('servers.console.send', $server) }}"
              method="POST"
              class="flex gap-2"
              autocomplete="off">
            @csrf
        ...
        @if (session('status'))
            <p class="text-xs text-emerald-300 mt-1">{{ session('status') }}</p>
        @endif
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const applyFilter = () => {
                ...
                if (filter === 'player') {
                    ...
                    lines = lines.filter(line => {
                        if (line.includes('[Rcon]')) return true;
